...
class FormManageNameServices {
  Future<List<TaskCode>> fetchTaskCodes({required BuildContext context}) async {
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    List<TaskCode> codesList = [];

    try {
      http.Response res =
          await http.get(Uri.parse('$uri/api/task-codes/getAll'), headers: {
      ...
      httpErrorHandle(
          ...
          onSuccess: () {
            for (int i = 0; i < jsonDecode(res.body).length; i++) {
              codesList
                  // .add(TaskCode.fromJson(jsonEncode(jsonDecode(res.body)[i])));
                  .add(TaskCode.fromJson(jsonDecode(res.body)[i]));
